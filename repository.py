...
class Repository:
    ...
    def get_event_by_id(self, event_id):
        ...
        try:
            ...
            if conn:
                ps_cursor = conn.cursor()
                ps_cursor.execute(f"Select event_id, title, image, username from events where eventid = {event_id}")
                event_records = ps_cursor.fetchall()
                for row in event_records:
    ...
    def get_event_by_title(self, title):
        ...
        try:
            ...
            if conn:
                ps_cursor = conn.cursor()
                ps_cursor.execute(f"Select event_id, title, image, username from event where title similar to '{title}%'")
                event_records = ps_cursor.fetchall()
                for row in event_records:
                    event = EventModel(id=row[0], title=row[1], likes=0, image=row[2], user_id=row[3])
                ps_cursor.close()
        ...
        try:
            ...
            if conn:
                ...
                if 'image' not in data:
                    data['image'] = ''
                # print(data)
                ps_cursor.execute(f"INSERT INTO event(title, username, likes, image) VALUES ('{data['title']}', '{data['user_id']}', '{data['image']}') RETURNING eventid")
                conn.commit()
